    ...
    if executable_exists("pdftotext"):
        try:
            ...
            current_app.logger.error(
                "Unable to read from file %s. (%s)" % (document, ex1.strerror)
            )
    else:
...
def get_plaintext_document_body(fpath, keep_layout=False):
    ...
    if os.access(fpath, os.F_OK | os.R_OK):
        ...
        # get file type:
        cmd_pdftotext = [current_app.config.get("CLASSIFIER_PATH_GFILE"), fpath]
...
def convert_PDF_to_plaintext(fpath, keep_layout=False):
    ...
    cmd_pdftotext = [
        current_app.config.get("CLASSIFIER_PATH_PDFTOTEXT"),
        layout_option,
        "-q",
        "-enc",
        "UTF-8",
